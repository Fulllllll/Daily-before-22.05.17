...
bool LoadFlightList(FlightList* flight)
{
	...
	while (strcmp(buf, "飞机") == 0 && (!feof(fp)))
	{
		...
		temp->Lhead = (Waitnode*)malloc(sizeof(Waitnode));
		temp->Lhead->next = NULL;
		fscanf(fp, "%s\n", buf);
	}
	fclose(fp);
	return 1;
...
bool LoadOrders(NormalOrderList* ODList, FlightList* flight)
{
	...
	while (fscanf(fp, "%d\n", &flag) && (!feof(fp)))
	{
		...
		else
		{
			...
			fscanf(fp, "C:%d\n", &(p->order.C));
			p->next = flight->List[i].Lhead->next;
			flight->List[i].Lhead->next = p;
		}
	}
	return 1;
}
...
bool SaveOrders(NormalOrderList* ODList, FlightList* flight)
{
	...
	{	
		...
		fprintf(fp, "A:%d ", ODList->List[i].B);
		fprintf(fp, "A:%d\n", ODList->List[i].C);
	}
...
bool DelOrders(NormalOrderList* ODList, FlightList* flight)
{
	...
	for (int i = 0; i < flight->sz; i++)	
	{
		...
		while (p)
		{
			flight->List[i].Lhead->next = p->next;
			free(p);
			p = NULL;
		}
		free(flight->List[i].Lhead);
		flight->List[i].Lhead = NULL;
	}
	return 1;
}
